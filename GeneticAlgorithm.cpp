...
void initializePopulation(Candidate *&population, bool varyNumOfLinks, bool varyWeightOfLinks, bool varyLengthOfLinks)
{
  ...
  std::uniform_real_distribution<double> torqueDistribution(TORQUE_LOWER_BOUND,TORQUE_UPPER_BOUND + 1);
  std::uniform_real_distribution<double> varyLinksDistribution(NUM_OF_LINKS_LOWER_BOUND,NUM_OF_LINKS_UPPER_BOUND + 1);
  std::uniform_real_distribution<double> varyWeightDistribution(WEIGHT_OF_LINK_LOWER_BOUND,WEIGHT_OF_LINK_UPPER_BOUND+ 1);
  std::uniform_real_distribution<double> varyLengthDistribution(LENGTH_OF_LINK_LOWER_BOUND,LENGTH_OF_LINK_UPPER_BOUND + 1);
...
void evaluatePopulation(Candidate *&population)
{
  for(int i=0; i < POPULATION_SIZE; i++){
    ...
    cout << "Evaluated population, at " << i << ": " << population[i].fitness <<  endl;
